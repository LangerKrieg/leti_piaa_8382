...
public:
    void readData(){
        ...
        while(!flag){
            //если кол-во блоков меньше= 0; размер паттерна больше 1, а кол-во блоков слишком велико
            //т.е. при таком размере пересечений при разбиении может быть утерян какой-то символ из паттерна;
            //или размер паттерна = 1, тогда кол-во блоков не может быть больше размера текста
            if(B <= 0 || (sizePattern > 1 && B > (sizeText / (sizePattern - 1)))
